...
const ProjectEdit = (props) => {
	...
	const { current: project, data, loading } = useSelector((state) => state.project)
	const [countResume, setCountResume] = useState(props?.data?.resume?.length || 0)
				...
				<div className={styles.label}>
					...
					<InputGroup style={{ flexWrap: "nowrap", width: "100%" }}>
						...
						{pitch?.length > 1 && (
							<RemoveGroup
								onClick={() =>
									setPitchs((state) => [...state].slice(0, -1))
								}
