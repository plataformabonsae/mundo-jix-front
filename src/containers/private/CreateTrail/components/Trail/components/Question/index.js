...
import { question, up, down } from "services/createTrail";
...
const Question = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    dispatch(
      ...
      .then((res) => {
        setTrailId(res.data.data.question.id);
        console.log(res);
        console.log(res.data.data.question.id);
      })
      .then((res) => {
    ...
    <Card border>
      <form noValidate onSubmit={handleSubmit(onSubmit)}>
        <header className={styles.header}>
          <Title style={{ marginBottom: 32 }}>{index + 1}) Questão</Title>
