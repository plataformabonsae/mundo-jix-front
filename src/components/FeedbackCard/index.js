...
import { comment, getOne, create, getAsMentor as get, reset } from "services/feedbacks"
...
const Modal = (props) => {
	...
	const { current: feedback, loading } = useSelector((state) => state.feedbacks)
	// const { data: usertype } = useSelector(state => state.usertype)

	useEffect(() => {
		dispatch(getOne(usertype, { challenge_id: challengeId, feedback_id: data.id }))
			...
			<article className={styles.modal}>
				...
				<form noValidate onSubmit={handleSubmit(onSubmit)}>
					...
					<input
						...
						value={user?.user?.id}
					/>
					{console.log("usuario", user?.badges[0].pivot.company_id)}
