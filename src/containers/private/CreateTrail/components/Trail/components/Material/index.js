...
import { material, up, down } from "services/createTrail";
...
const Material = (props) => {
  ...
  const onSubmit = async (data) => {
    ...
    dispatch(
      material(usertype, { challenge_id: id, ...data, file: data.file[0] })
    )
      .then((res) => {
        setTrailId(res.data.data.material.id);
        console.log(res);
        console.log(res.data.data.material.id);
      })
      .then((res) => {
      ...
      .catch((error) => {
        toast.error("Um erro ocorreu ao salvar a trilha.", {
          position: toast.POSITION.BOTTOM_RIGHT,
        });
    ...
    <Card border>
      <form noValidate onSubmit={handleSubmit(onSubmit)}>
        <header className={styles.header}>
          <Title style={{ marginBottom: 32 }}>{index + 1}) Material</Title>
