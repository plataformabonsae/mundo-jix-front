...
import { edit, get } from "services/auth"
...
const Pessoal = ({ type, noShadow, finalRoute, hasPassword, redirect, dontRedirect }) => {
	...
	const handleCancelSubscriptionModal = () =>
		setCancelSubscriptionModal((prev) => !prev)

	const handleCancelSubscription = async () =>
		dispatch(cancelSubscription(usertype))
			.then(() => {
		...
		return (
			<>
				<form noValidate onSubmit={handleSubmit(onSubmit)}>
					<Card noShadow={noShadow}>
						...
						<InputGroup>
							<InputWithMask
								...
								control={control}
								mask={`999.999.999-99`}
								disabled={hasPassword ? true : false}
							>
								CPF
							</InputWithMask>
							<InputWithMask
					...
					<Card noShadow={noShadow}>
						...
						<InputGroup style={{ flexWrap: "nowrap", width: "100%" }}>
							...
							{emails?.length > 0 && (
								<RemoveGroup
									onClick={() =>
										setEmails((state) => [...state].slice(0, -1))
									}
									text="Remover e-mail"
								/>
							)}
					...
					<Card noShadow={noShadow}>
						...
						<InputGroup style={{ flexWrap: "nowrap", width: "100%" }}>
							...
							{socials?.length > 1 && (
								<RemoveGroup
									onClick={() =>
										setSocials((state) => [...state].slice(0, -1))
									}
					...
					{!!user.accepted_terms && (
						<Card noShadow={noShadow}>
							<Title style={{ marginBottom: 32 }}>Assinatura</Title>
							{!!user?.is_subscribed ? (
								<>
									<Text style={{ marginBottom: 24 }}>
										Você tem uma assinatura de Autodesafios ativa no
										valor de R$27,90 por mês.
									</Text>
									<Button
								...
								<Text style={{ marginBottom: 24 }}>
									Você não tem uma assinatura ativa. Para adquirir uma e
									ter acesso a todos os Autodesafios, clique em
									"participar" em qualquer Autodesafio.
								</Text>
							)}
						</Card>
					)}
				...
				{cancelSubscriptionModal && (
					<Dialog
						...
						handleClose={handleCancelSubscriptionModal}
					>
						<Title size={18} style={{ marginTop: 24 }}>
							Deseja cancelar a sua inscrição nos autodesafios?
						</Title>
						<ButtonGroup>
							<Button
								Tag={"span"}
	...
	return (
		...
		>
			<form noValidate onSubmit={handleSubmit(onSubmit)}>
				...
				<Button submit disabled={loading || !allValid} Tag="button" type="green">
					Atualizar senha
				</Button>
			</form>
		</Dialog>
