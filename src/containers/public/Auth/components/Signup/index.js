...
const Email = ({ title, desc, type }) => {
	...
	useEffect(() => {
		if (linkedinCallBack?.code) {
			...
			req.then((response) => {
				const res = dispatch(tokenFetchExternal(type, { email: response.email }))
				res.then((responseApi) => {
	...
	const responseGoogle = async (data) => {
		const res = dispatch(tokenFetchExternal(type, { email: data?.profileObj?.email }))
		await res.then((response) => {
			response?.data?.message === "User Not Found!" &&
				dispatch(
					newuser(type, {
	...
	return (
        ...
        `}
				...
				<Button
					disabled={
						loading || errors?.email || !name.length || type === "talento"
							? !lastName.length
							: false || !allValid
						// !email.length
					}
