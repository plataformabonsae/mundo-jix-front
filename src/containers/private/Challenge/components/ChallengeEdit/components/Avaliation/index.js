...
const Avaliation = ({ noShadow = true, handleClose }) => {
	...
	const handleShowEditAvaliations = () => setShowEditAvaliations((prev) => !prev)
		...
		return (
			<>
				<form noValidate onSubmit={handleSubmit(onSubmit)}>
					...
					<input
						type="hidden"
						value={challenge.challenge.deadline}
						ref={register()}
						name={"deadline"}
					/>
					<Card noShadow={noShadow}>
						...
						<Text style={{ marginBottom: 24 }}>
							Ao editar a avaliação, as avaliações já feitas pelos jurados
							serão perdidas.
						</Text>
												...
												<Input
													...
													ref={register({
														required: {
															value: true,
															message:
																"Digite o que deseja avaliar",
														},
													})}
													validate={
														errors?.assessments?.[index]
															?.evaluate?.message
													}
